...
export function CheckoutClient({
  ...
  // Simulación de acción al pulsar el botón oficial de PayPal
  const handlePayPalPayment = async () => {
    setStep("processing");
    setError(null);

    try {
      // Simula la llamada a tu API backend que inicializa el Order de PayPal
      await new Promise((resolve) => setTimeout(resolve, 2500));

      // Simulación de éxito
      setStep("success");
    } catch (err) {
      setError("Não foi possível conectar com o PayPal. Por favor, tenta novamente.");
      setStep("error");
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── STEP: Selección / Botón de Pago PayPal ── */}
      {step === "select" && (
        <div className="space-y-4 animate-fade-in">
          <div className="rounded-2xl border border-gray-100 bg-gray-50/50 p-5 text-center">
            <p className="text-xs text-gray-500 font-medium mb-4">
              Ao clicar no botão abaixo, serás redirecionado com segurança para o ambiente oficial do **PayPal** para concluir a tua transação.
            </p>

            {/* Botón Estilizado Oficial de PayPal */}
            <button
              type="button"
              onClick={handlePayPalPayment}
              className="w-full bg-[#ffc439] hover:bg-[#f2ba13] text-[#003087] font-black py-3.5 px-6 rounded-xl transition duration-200 flex items-center justify-center gap-2 shadow-md shadow-yellow-500/10"
            >
              <span className="italic text-base font-extrabold tracking-tight">Pay</span>
              <span className="italic text-base font-extrabold tracking-tight text-[#0079c1]">Pal</span>
            </button>
          </div>

          <div className="flex items-center justify-center gap-2 text-[11px] font-bold text-gray-400">
            <Lock className="h-3 w-3" /> Transação encriptada com proteção ao comprador
          </div>
        </div>
      )}
      ...
      {step === "processing" && (
        <div className="flex flex-col items-center gap-4 py-12 text-center animate-fade-in">
          ...
          <div>
            <h3 className="text-sm font-black text-gray-800 uppercase tracking-wider">A processar o teu pagamento</h3>
            <p className="mt-1 text-xs text-gray-400 font-medium">
              Estamos a estabelecer ligação segura com o PayPal. Não feches esta janela.
            </p>
          </div>
        </div>
      )}

      {/* ── STEP: Éxito / Confirmación ── */}
      {step === "success" && (
        <div className="flex flex-col items-center gap-4 py-8 text-center animate-fade-in">
          ...
          <div>
            <h3 className="text-base font-black text-gray-900 uppercase tracking-tight">Compra concluída com sucesso!</h3>
            <p className="mt-1 text-xs text-gray-500 font-medium px-4">
              Obrigado pela tua compra. Os teus **{quantity}** bilhetes para **{eventTitle}** foram gerados e enviados para o teu e-mail de registo.
            </p>
          </div>
      ...
      {step === "error" && (
        <div className="flex flex-col items-center gap-4 py-8 text-center animate-fade-in">
          ...
          <div>
            <h3 className="text-base font-black text-gray-900 uppercase tracking-tight">Pagamento não concluído</h3>
